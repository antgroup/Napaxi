...
static RE_CONTEXT_LENGTH: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)maximum context length is\s+(\d+)\s*token").expect("static regex is valid")
});
...
static RE_PROMPT_TOKENS_PREFIXED: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)(\d+)\s+in the (?:prompt|messages?)").expect("static regex is valid")
});

static RE_PROMPT_CHARS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)prompt\s+contains?\s+(\d+)\s*character").expect("static regex is valid")
});
