...
impl EngineHandle {
    ...
    pub fn configure_git_identity(self, name: &str, email: &str) -> CoreResult<()> {
        // SAFETY: `self.raw()` is an engine handle created by this runtime; an
        // invalid or disposed handle is converted into `CoreError::InvalidHandle`.
        let engine =
...
mod tests {
    ...
    fn invalid_handle_identity_methods_reject() {
        ...
        assert_eq!(h.read_git_identity().unwrap_err().code(), "invalid_handle");
    }
}
