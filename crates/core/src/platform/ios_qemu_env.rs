...
pub mod pty {
    ...
    use super::{
        ensure_setup_and_init_with_workspace_dir, normalize_workdir, qemu_env_json,
        qemu_session_close, qemu_session_open, qemu_session_read, qemu_session_resize,
        qemu_session_wait_output, qemu_session_write,
    };
    ...
    pub fn resize_pty_session(session_id: u64, cols: u16, rows: u16) -> anyhow::Result<()> {
        ...
        let rc = unsafe {
            qemu_session_resize(session_id, i32::from(cols.max(1)), i32::from(rows.max(1)))
        };
        if rc != 0 {
    ...
    fn read_session_loop(session_id: u64, event_tx: mpsc::Sender<PtyEvent>) {
        ...
        loop {
            ...
            loop {
                ...
                let read =
                    unsafe { qemu_session_read(session_id, buffer.as_mut_ptr(), buffer.len()) };
                if read > 0 {
