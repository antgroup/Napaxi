...
fn run_pty_session(
    ...
    #[cfg(unix)]
    {
        ...
        // SAFETY: Each descriptor is a fresh duplicate owned by this process;
        // `Stdio` takes ownership and will close it with the child stream.
        command.stdin(unsafe { Stdio::from_raw_fd(slave_stdin) });
        // SAFETY: Each descriptor is a fresh duplicate owned by this process;
        // `Stdio` takes ownership and will close it with the child stream.
        command.stdout(unsafe { Stdio::from_raw_fd(slave_stdout) });
        // SAFETY: `slave_fd` is the original slave PTY descriptor and is handed
        // to `Stdio`, which assumes ownership for the child stderr stream.
        ...
        // SAFETY: `master_for_write` is a fresh duplicate owned by this thread;
        // converting it to `File` transfers ownership for RAII close on drop.
...
fn configure_pty_child_process(command: &mut std::process::Command, slave_ctty: i32) {
    ...
    // async-signal-safe libc functions (`setsid`, `ioctl`) and returns errors
    // through `std::io::Error::last_os_error`.
    unsafe {
...
fn open_host_pty(cols: u16, rows: u16) -> anyhow::Result<(i32, i32)> {
    ...
    let mut master = -1;
    let mut slave = -1;
    // SAFETY: `master` and `slave` are valid out-pointers for `openpty`, and
    // `winsize` lives for the duration of the call.
    let result = unsafe {
        openpty(
            &mut master,
            &mut slave,
...
fn dup_fd(fd: i32) -> anyhow::Result<i32> {
    // SAFETY: `dup` does not take ownership of `fd`; it returns a new descriptor
    // or a negative error code, which is checked below.
...
fn close_fd(fd: i32) {
    if fd >= 0 {
        // SAFETY: Closing an owned raw descriptor is safe; duplicate close is
        // avoided by callers transferring ownership exactly once.
        unsafe {
            let _ = close(fd);
        }
    }
...
fn set_pty_size(fd: i32, cols: u16, rows: u16) {
    ...
    // SAFETY: `fd` is an open PTY descriptor and `winsize` is a valid pointer
    // for the duration of the ioctl call. Errors are intentionally ignored.
    unsafe {
...
fn spawn_pty_reader(session_id: u64, master_fd: i32, event_tx: Sender<PtyEvent>) -> JoinHandle<()> {
    thread::spawn(move || {
        // SAFETY: `master_fd` is moved into this reader thread and converted
        // exactly once into `File`, which owns and closes it on drop.
