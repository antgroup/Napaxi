...
pub(super) fn dispatch(handle: i64, namespace: &str, method: &str, payload: &Value) -> String {
    if namespace == "channel"
        && let Some(response) = super::channel_dispatch::dispatch_channel(handle, method, payload)
    {
        return response;
    }
    ...
    if namespace == "tools"
        && let Some(response) = super::tools_dispatch::dispatch_tools(handle, method, payload)
    {
        return response;
    }
    ...
    match (namespace, method) {
        ...
        ("agent_app", "delete_package") => ok(json!(
            napaxi_core::api::agent_app::delete_agent_app_package(
                handle,
                &get_string(payload, "agent_id"),
            )
        )),
        ...
        ("agent_app", "list_proposals") => ok_raw(
            napaxi_core::api::agent_app::list_agent_app_action_proposals(
                handle,
                &get_string(payload, "agent_id"),
            ),
        ),
        ...
        ("agent_defs", "import_markdown") => {
            ok_raw(napaxi_core::api::agent::import_agent_md_handle(
                handle,
        ...
        ("evolution", "reject_pending") => ok_raw(
            napaxi_core::api::evolution::reject_pending_evolution_handle(
                handle,
        ...
        ("workspace", "system_prompt") => {
            ok_raw(napaxi_core::api::workspace::system_prompt_handle(
                handle,
                &get_string(payload, "account_id"),
                &get_string(payload, "agent_id"),
            ))
        ...
        ("workspace", "search_memory") => {
            ok_raw(napaxi_core::api::workspace::search_memory_handle(
                handle,
                &get_string(payload, "account_id"),
                &get_string(payload, "agent_id"),
        ...
        ("file_bridge", "sandbox_to_real") => ok(json!(
            napaxi_core::api::file_bridge::sandbox_to_real_handle(
                handle,
        ...
        ("file_bridge", "real_to_sandbox") => ok(json!(
            napaxi_core::api::file_bridge::real_to_sandbox_handle(
                handle,
