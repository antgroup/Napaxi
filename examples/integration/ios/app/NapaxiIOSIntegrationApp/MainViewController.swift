...
final class MainViewController: UIViewController {
    ...
    private func buildStatusSummary() -> String {
        do {
            ...
            let engine = try Self.makeEngineForRelease(filesDir: filesDir, qemuReady: qemuReady)
            let shellProbe = qemuReady ? try Self.runQemuShellProbe(engine: engine) : "skipped: qemuReady=false"

            return [
                "Napaxi iOS release is ready.",
            ...
            return "Napaxi iOS release failed: \(error)"
        }
    }

    static func makeFilesDir() -> String {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("napaxi-ios-app-release", isDirectory: true)
            .path
    }
    ...
    static func makeEngineForRelease(filesDir: String, qemuReady: Bool = NapaxiIosQemuSandboxSupport.isBundledSandboxAvailable) throws -> NapaxiEngine {
    ...
    static func runQemuShellProbe(engine: NapaxiEngine) throws -> String {
        let arguments: [String: NapaxiJSONValue] = [
            "command": .string("echo napaxi-ios-release && pwd && uname -m"),
            "timeout": .number(20),
        ]
        let request = try NapaxiRawJSON(.object([
            "call_id": .string("ios-release-shell-probe"),
            "name": .string("shell"),
    ...
    private static func writeStatusReport(_ summary: String) {
        do {
            ...
            NSLog("Napaxi iOS release report write failed: \(error)")
        }
    }
}
...
final class AppApprovalHandler: NapaxiStructuredToolApprovalHandler {
    func approve(_ request: NapaxiHostToolApprovalRequest) async -> NapaxiHostToolApprovalResponse {
        NapaxiHostToolApprovalResponse(approved: true, message: "Approved by Napaxi iOS release app")
    }
}
