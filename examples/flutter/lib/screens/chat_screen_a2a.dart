...
mixin _ChatScreenA2AMixin on State<ChatScreen> {
  ...
  bool get _usesInjectedChatClient;
  ...
  Future<sdk.A2APeerMessage?> _a2aOriginalTaskRequestMessage(
    ...
    final sessionId = taskRecord.sessionId?.trim() ?? '';
    if (sessionId.isEmpty) return null;
    final messages = await client.listLocalA2APeerMessages(
      sessionId,
      limit: 50,
    );
    for (final task in messages) {
      if (task.kind != 'task_request') continue;
      if (_a2aTaskIdFromMessage(task) == taskRecord.taskId) return task;
    }
    return null;
  }
  ...
  String _a2aSendFailureMessage(
    ...
    return [
      ...
      if (message.sessionId.trim().isNotEmpty)
        '也可用 `/a2a trace ${message.sessionId}` 查看链路证据。',
    ].join('\n');
  }
  ...
  Future<void> _ensureA2AConnectionReadyForUserTurn() async {
    if (_usesInjectedChatClient) return;
  ...
  void _appendA2APairingRequestNotice(sdk.A2APeerMessage message) {
    ...
    _appendA2ANoticeMessage(
      [
        ...
        '也可以执行 `/a2a pair-accept ${message.fromPeerId} <对方配对密钥>` 完成确认。',
        '注意：这只确认本机信任对端，不会让对端自动信任本机。',
      ].join('\n'),
    );
  }
      ...
      if (!messages.any((message) => message.id == stableMessageId)) {
        ...
        messages.add(
          ChatMessage(
            ...
            createdAt: now,
            reasoning: '正在思考回复。\n',
            traceSteps: const [AgentTraceStep(reasoning: '正在思考回复。\n')],
            isStreaming: true,
          ),
        );
      }
